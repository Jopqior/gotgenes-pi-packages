...
A fork PR's `statusCheckRollup` can be **empty** because the run awaits maintainer approval or has not been created yet (upstream observed a delay of ~4 minutes in gotgenes/pi-packages#959).
Absent checks mean *not run*, never *passed*; do not read `mergeable`/`mergeStateStatus` as evidence of a green build.
Tell them apart with `gh api "repos/Jopqior/gotgenes-pi-packages/actions/runs?head_sha=<sha>" --jq .total_count`: `0` is not-yet-created, and an `action_required` run needs `gh api -X POST repos/Jopqior/gotgenes-pi-packages/actions/runs/<id>/approve`.
