...
- Hide denied tools from the agent before it starts (tool filtering + the tool-surface prompt pass), but only when the surface is *fully* denied.
  ...
  Removal reaches only text pi or this package wrote, which is why the prompt is split at pi's `Current working directory:` footer — written last and unconditionally in both branches, the anchor `pi-subagents` uses for the skills catalogue.
  Above it, a section is removed only when `ToolSurfaceInputs.piAuthoredPreamble` says pi wrote the preamble (`!systemPromptOptions.customPrompt`); below it, always, since that region holds this package's own block and any peer's under #901; and a section never extends past its own body, so a match cannot sweep the prose after it.
  Under a custom prompt pi writes **no** tool surface at all, so every match above the footer is a user's or another extension's — removing it deleted a `SYSTEM.md`'s own guidelines and pi's `<project_context>` opening tag (Refs #919, #932).
  Do not branch the whole pass on `customPrompt`: every pi-subagents child is a `customPrompt` session too, so skipping it there leaves every child with no tool prose.
...
- `path` and `external_directory` each carry a **read/write axis** (ADR 0013 §3–§4, Refs #806), and the two directions are independent bits, not tiers — a `path_write` allow grants no read, a `path_read` deny floors no write.
  ...
  `unresolved-salvage.ts` is `parseUnresolvedWithin`'s third caller, asking it twice over: to locate the innermost unresolved region and to refuse the re-parse of that region when it failed too (#875).
...
A locally-adjudicating child skipping a configured link whose provider did not load there keeps skipping it (the conflict is between two operator statements, and this package resolves it in favor of the load-level exclusion), but no longer silently: `AuthorizerChainAudit` (`src/authority/authorizer-chain-audit.ts`) writes the per-ask `authorizer_chain_unregistered_link` record **and** warns once per configured name, since the operator asked for that judge and the review log alone never said they did not get it (#861).
...
That whole-command resolve runs whenever the **primary** parse matched nothing, including when the salvage (#875) then recovered units from the wreckage; only the synthetic ask is skipped there.
...
Since #875 the **enumeration** half is closed too, and it was the wider hole: the dropped region lost its path operands as well as its command unit, so `cat <<'MSG' 2>&1 | cat /etc/shadow` reached no `bash:` rule, no `path` rule, and no `external_directory` gate — an ADR 0009 completeness-contract violation rather than a residual.
`withSalvagedRoots` (`access-intent/bash/unresolved-salvage.ts`) finds the innermost node whose subtree `parseUnresolvedWithin` reports, re-parses **its own source text** standalone, and admits the result **only when that re-parse is clean**; `BashProgram.parse` and `parseBashCommandsSync` then enumerate each salvaged root through `collectSalvagedCommands` (marked `parseUnresolved`, so the floor still clamps an `allow`) and `BashPathResolver.resolve(root, salvagedRoots)` collects its tokens into the same candidate array before projection, so a path both name folds to one prompt entry.
The clean-re-parse condition is the whole safety argument and must not be relaxed: recovery invents the structure inside an unresolved region (#742) and invented structure does not re-parse — measured, dropping it emits `">"` as a command unit for `cat <> rw.txt`.
A salvaged root is walked under the **unknown** effective base, never the cwd: the fragment carries no record of the `cd` in force where it sat, and resolving `cat ../secret` after `cd /outside` against the cwd would name a different file than the one that runs (the residual is recorded in ADR 0009).
The trigger stays the parse's health rather than a node type — every salvageable region of the one grammar gap met so far is a `file_redirect` across 25 probed spellings, so no real command falsifies the narrower rule and a stub-node test pins it instead; a node-type trigger would silently drop the next gap.
A recovered unit also carries `salvaged: true`, narrower than `parseUnresolved`, and `resolveBashCommandCheck` keys the #452 whole-command `deny` probe on the **primary** parse having matched nothing rather than on the unit list being empty: `> f <<'M' 2>&1 | rm -rf /tmp/x` yields zero primary units and one salvaged one, so keying on the combined list turned a `deny` on a context-naming rule (`"* rm -rf *"`) into an `ask`.
Do not re-key that branch on `commands.length === 0`.
What remains is a region whose own text does not re-parse either, which the #840 floor prompts for by naming the whole command line.
