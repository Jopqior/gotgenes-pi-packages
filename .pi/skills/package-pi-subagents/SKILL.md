...
The extension is organized into seven domains (73 files):
...
| Lifecycle   | `subagent-manager.ts`, `create-subagent-session.ts`, `subagent-session.ts`, `turn-limits.ts`, `subagent.ts`, `subagent-state.ts`, `run-listeners.ts`, `workspace-bracket.ts`, `concurrency-limiter.ts`, `parent-snapshot.ts`, `child-lifecycle.ts`, `child-shutdown.ts`, `workspace.ts`, `selection-scope.ts`, `spawn-selection.ts`, `initial-spawn-selection.ts`, `usage.ts` | 17      | Spawn, abort, resume, concurrency admission, session assembly factory, born-complete turn loop, status state machine, lifecycle-state value object, per-subagent behavior, per-run listener handles, workspace prepare/dispose lifecycle, child-lifecycle events, child extension shutdown on disposal, workspace provider seam, spawn-selection lease, and initial selection attempt/acknowledgement owner |
...
SubagentManager ─constructs─→ InitialSpawnSelection ←delegates─ Subagent
...
- **Spawn selection** — `registerSpawnSelectionProvider` is an optional generative seam.
  ...
  `InitialSpawnSelection` owns the private one-shot outcome, pending activity, confirmed pair, provider cancellation race, and startup listeners, detached at confirmation; the manager constructs it from the spawn identity, snapshot catalogue, and retained scope.
  The record delegates selection after admission, rechecks the permit after workspace preparation, and supplies recorded terminal facts after the original initial-run observer; cancellation must release queued or active selection even when the provider ignores abort.
