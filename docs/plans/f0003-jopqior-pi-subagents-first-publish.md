...
The "until [#3] those two fields still match upstream, so this is a no-op" sentence in that handbook becomes false when `name` / `version` diverge and must be rewritten in this issue.
...
| `cliff.toml`                                                         | `<REPO>` postprocessor → `https://github.com/Jopqior/gotgenes-pi-packages`. Historical 21.x changelog URLs stay baked. Post-1.0.0 git-cliff sections then link fork issues; squash-sync is a single fork-authored commit, so upstream `(#N)` subjects are not replayed as separate changelog rows. |
...
### Handbook sentence (with the rename)

In `docs/upstream-sync.md` **`packages/pi-subagents/package.json`** recipe, drop "Until [#3] those two fields still match upstream, so this is a no-op until the rename."
The recipe already says take theirs then restore `name` and `version` from ours; that is now load-bearing.
...
- `docs/upstream-sync.md` — drop the "no-op until rename" sentence in the package.json recipe (rename commit); add the correspondence row (post-tag commit).
...
2. **Published identity docs and cliff `<REPO>`** — package README (H1, badges, install, service/settings examples), `docs/configuration.md` opening sentence, the three JSDoc/comment examples, `cliff.toml` postprocessor, root README Packages-table row, and the `docs/upstream-sync.md` package.json recipe sentence (remove the "no-op until rename" clause).
