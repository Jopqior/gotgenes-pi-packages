...
- **Accepted residual:** a portable child whose parent has no custom prompt or append prompt falls back to `genericBase`, which claims write and exec capability a read-only child does not hold.
  Tracked as [#904].
