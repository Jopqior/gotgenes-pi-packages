...
- `workspace` — a generative seam ([ADR-0002]): a registered `WorkspaceProvider` supplies a child's cwd plus bracketed `dispose()` at run-start.
...
2. **Provider seams (generative) — rationed.**
   ...
   The current fork supports the **workspace provider** (returns the child's working directory plus bracketed setup/teardown) and the optional **spawn-selection provider** (returns the model/thinking pair before workspace or session creation).
...
The architecture above incorporates inherited upstream numeric Phases 1–22 and the fork's independent Phase f1; upstream Phase 6 (UI extraction to a separate package) was folded into [ADR-0004] rather than executed.
Each completed phase's findings, plan, dependency diagram, and health metrics are preserved in a per-phase history file under [`history/`](history/).
...
| f1    | Upstream integration maintenance — Complete                  | [phase-f1-upstream-integration-maintenance.md](history/phase-f1-upstream-integration-maintenance.md) |
...
| Fork f1                       | [#20], [#21]                                                                                                                       | Initial-selection ownership and shared raw-fact presentation                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Fork f1 (combined assessment) | [#19]                                                                                                                              | Operator-accepted, bounded reduction in upstream reconciliation obligations                                                                                                                                                                                                                                                                                                                                                                                                                    |
